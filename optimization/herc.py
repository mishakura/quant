...
for rm in RISK_MEASURES:
    try:
        ...
        results[rm] = w
        print(f"\nOptimal herc Portfolio Weights for {rm}:")
        print(w)
...
with pd.ExcelWriter("herc_portfolio_weights_all_rm.xlsx") as writer:
    for rm, w in results.items():
        w.to_excel(writer, sheet_name=rm)
    # Save stats as a new sheet
    stats_df = pd.DataFrame(stats)
    stats_df.to_excel(writer, sheet_name="Stats", index=False)
print("\nAll herc weights and stats saved to herc_portfolio_weights_all_rm.xlsx")
